    ...
    <div style={{position:'fixed',inset:0,background:'#000c',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:'#181c27',border:'1px solid #2a3050',borderRadius:16,width:'100%',maxWidth:560,maxHeight:'90vh',overflowY:'auto'}}>
        <div style={{padding:'16px 20px',borderBottom:'1px solid #2a3050',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div>
            <div style={{fontWeight:800,fontSize:15,color:'#e8eaf6'}}>{entry.doc_number ? `№${entry.doc_number}` : 'Документ'} — Проверка</div>
            <div style={{fontSize:12,color:'#8892b0',marginTop:3}}>{entry.counterparty||'—'} · {entry.entry_date}</div>
          </div>
        ...
        <div style={{padding:'16px 20px'}}>
          {entry.ai_reasoning && (
            <div style={{background:'#1e2640',borderRadius:8,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#8892b0'}}>
              🤖 <strong style={{color:'#818CF8'}}>AI:</strong> {entry.ai_reasoning}
              <div style={{marginTop:4,color:entry.ai_confidence>=80?'#10B981':'#F59E0B',fontWeight:700}}>Уверенность: {entry.ai_confidence}%</div>
            </div>
          )}
          <div style={{background:'#0f1117',borderRadius:10,padding:'12px 16px',marginBottom:16}}>
            <div style={{fontSize:11,color:'#4a5580',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10}}>Предложенная проводка</div>
          ...
          {mode === 'correct' && (
            <div style={{marginBottom:16}}>
              <div style={{fontSize:12,color:'#F59E0B',fontWeight:700,marginBottom:12}}>✏️ Исправление</div>
              <div style={{marginBottom:10}}>
                <label style={LBL}>Поиск счёта</label>
                <input value={acctSearch} onChange={e=>setAcctSearch(e.target.value)} placeholder="Код или название..." style={INP}/>
                {acctSearch && filtered.length > 0 && (
                  <div style={{background:'#0f1117',border:'1px solid #2a3050',borderRadius:8,marginTop:4,maxHeight:160,overflowY:'auto'}}>
                    {filtered.map(a => (
                      <div key={a.code} style={{display:'flex',justifyContent:'space-between',padding:'8px 12px',borderBottom:'1px solid #1e2640',fontSize:12}}>
                        <span style={{color:'#818CF8',fontWeight:700,marginRight:8}}>{a.code}</span>
                        <span style={{color:'#8892b0',flex:1}}>{a.name}</span>
                        <div style={{display:'flex',gap:6}}>
                          <button onClick={()=>{setDebit(a.code);setAcctSearch('')}} style={{background:'#4F46E522',border:'1px solid #4F46E544',color:'#818CF8',fontSize:10,padding:'2px 6px',borderRadius:4,cursor:'pointer',fontFamily:'inherit',fontWeight:700}}>Дт</button>
                          <button onClick={()=>{setCredit(a.code);setAcctSearch('')}} style={{background:'#10B98122',border:'1px solid #10B98144',color:'#34D399',fontSize:10,padding:'2px 6px',borderRadius:4,cursor:'pointer',fontFamily:'inherit',fontWeight:700}}>Кт</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
                <div><label style={LBL}>Дебет *</label><input value={debit} onChange={e=>setDebit(e.target.value)} placeholder="напр. 7220" style={{...INP,color:'#818CF8',fontWeight:700}}/></div>
                <div><label style={LBL}>Кредит *</label><input value={credit} onChange={e=>setCredit(e.target.value)} placeholder="напр. 3210" style={{...INP,color:'#34D399',fontWeight:700}}/></div>
              </div>
          ...
          {error && <div style={{background:'#3A0808',color:'#FCA5A5',fontSize:13,padding:'10px 12px',borderRadius:8,marginBottom:14}}>{error}</div>}

          {mode === 'view' && (
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>act('confirm')} disabled={saving} style={{flex:1,background:'#10B981',color:'#fff',border:'none',padding:12,borderRadius:10,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>✅ Подтвердить</button>
              <button onClick={()=>setMode('correct')} style={{flex:1,background:'#F59E0B22',color:'#F59E0B',border:'1px solid #F59E0B44',padding:12,borderRadius:10,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>✏️ Исправить</button>
              <button onClick={()=>setMode('reject')} style={{flex:1,background:'#EF444422',color:'#EF4444',border:'1px solid #EF444444',padding:12,borderRadius:10,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>❌ Отклонить</button>
            </div>
          )}
          {mode === 'correct' && (
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>act('correct')} disabled={saving||!debit||!credit} style={{flex:2,background:(!debit||!credit)?'#374151':'#F59E0B',color:(!debit||!credit)?'#6B7280':'#000',border:'none',padding:12,borderRadius:10,fontSize:14,fontWeight:800,cursor:(!debit||!credit)?'not-allowed':'pointer',fontFamily:'inherit'}}>{saving?'Сохраняю...':'✏️ Сохранить'}</button>
              <button onClick={()=>setMode('view')} style={{flex:1,background:'none',color:'#8892b0',border:'1px solid #2a3050',padding:12,borderRadius:10,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Назад</button>
            </div>
          )}
          {mode === 'reject' && (
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>act('reject')} disabled={saving} style={{flex:2,background:'#EF4444',color:'#fff',border:'none',padding:12,borderRadius:10,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>{saving?'Отклоняю...':'❌ Подтвердить'}</button>
              <button onClick={()=>setMode('view')} style={{flex:1,background:'none',color:'#8892b0',border:'1px solid #2a3050',padding:12,borderRadius:10,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Назад</button>
            </div>
          )}
        </div>
...
export default function Journal() {
  ...
  const cols = selectMode
    ? '28px 36px 70px 100px 130px 1fr 90px 90px 110px 100px'
    : '36px 70px 100px 130px 1fr 90px 90px 110px 100px'
    ...
    <div style={{background:'#0f1117',minHeight:'100vh',fontFamily:'Manrope, sans-serif',color:'#e8eaf6'}}>
      ...
      <div style={{background:'#181c27',borderBottom:'1px solid #2a3050',padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <button onClick={()=>navigate(`/company/${companyId}`)} style={{background:'none',border:'none',cursor:'pointer',fontSize:20,color:'#8892b0'}}>←</button>
          <div>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <button onClick={()=>navigate(`/company/${companyId}/scanner`)} style={{background:'#181c27',color:'#8892b0',border:'1px solid #2a3050',padding:'8px 14px',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>📷 Сканер</button>
          {selectMode ? (
            <>
              ...
              <button onClick={()=>{setSelectMode(false);setSelected(new Set())}} style={{background:'none',color:'#8892b0',border:'1px solid #2a3050',padding:'8px 14px',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Отмена</button>
            </>
          ) : (
            <>
              <button onClick={()=>setSelectMode(true)} style={{background:'#181c27',color:'#8892b0',border:'1px solid #2a3050',padding:'8px 14px',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>☑ Выбрать</button>
              <button onClick={runAutoAll} disabled={postingAll} style={{background:postingAll?'#374151':'#4F46E5',color:'#fff',border:'none',padding:'8px 14px',borderRadius:8,fontSize:12,fontWeight:700,cursor:postingAll?'not-allowed':'pointer',fontFamily:'inherit'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,padding:'16px 20px 0'}}>
        {[
          ...
          {label:'На проверке',value:needsReview,color:'#F59E0B',action:needsReview>0?()=>setFilterStatus('needs_review'):null},
          {label:'Итого KGS',value:totalKgs>0?fmt(totalKgs):'—',color:'#818CF8',small:true},
        ].map(s => (
          <div key={s.label} onClick={s.action||undefined}
            style={{background:'#181c27',borderRadius:10,padding:'12px 14px',border:`1px solid ${s.action?'#F59E0B44':'#2a3050'}`,cursor:s.action?'pointer':'default'}}>
          ...
          <>
            ...
            <div style={{marginBottom:12}}>
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                ...
                <button onClick={()=>setShowFilters(!showFilters)} style={{padding:'5px 12px',borderRadius:20,border:'1px solid #2a3050',background:'none',color:'#8892b0',fontSize:12,cursor:'pointer',fontFamily:'inherit',marginLeft:'auto'}}>
            ...
            {loading ? (
              ...
              <div style={{textAlign:'center',padding:48,background:'#181c27',borderRadius:14,border:'1px solid #2a3050'}}>
                ...
                <button onClick={()=>navigate(`/company/${companyId}/scanner`)} style={{background:'#4F46E5',color:'#fff',border:'none',padding:'10px 20px',borderRadius:8,fontWeight:700,cursor:'pointer',fontFamily:'inherit',fontSize:13}}>📷 Сканировать</button>
              </div>
            ) : (
              <div style={{background:'#181c27',borderRadius:12,border:'1px solid #2a3050',overflow:'hidden'}}>
                {/* Шапка таблицы */}
                <div style={{display:'grid',gridTemplateColumns:cols,gap:6,padding:'10px 14px',background:'#0f1117',fontSize:10,fontWeight:700,color:'#4a5580',textTransform:'uppercase',letterSpacing:'0.08em'}}>
                  {selectMode && (
                ...
                {entries.map(e => (
                  <div key={e.id}>
                    {/* Строка */}
                    <div style={{display:'grid',gridTemplateColumns:cols,gap:6,padding:'11px 14px',borderTop:'1px solid #1e2640',alignItems:'center',cursor:'pointer',background:selected.has(e.id)?'#1e0a0a':e.status==='needs_review'?'#1A1200':expanded===e.id?'#1e2640':'transparent'}}
                      onClick={()=>!selectMode&&setExpanded(expanded===e.id?null:e.id)}
                      onMouseEnter={ev=>{if(!selectMode)ev.currentTarget.style.background='#1a1f35'}}
                      ...
                      <div style={{textAlign:'right'}}>
                        ...
                        {e.currency!=='KGS'&&e.amount_kgs&&<div style={{fontSize:10,color:'#818CF8'}}>≈{Number(e.amount_kgs).toLocaleString('ru-RU')} KGS</div>}
                      </div>
                      <div style={{display:'flex',flexDirection:'column',gap:3,alignItems:'flex-start'}}>
                        <span style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:20,background:S_BG[e.status]||'#F3F4F622',color:S_COLOR[e.status]||'#6B7280',whiteSpace:'nowrap'}}>
                          {S_LABEL[e.status]||e.status}
                        </span>
                        {e.status==='needs_review'&&(
                          <button onClick={ev=>{ev.stopPropagation();setReviewEntry(e)}}
                            style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:6,background:'#F59E0B',color:'#000',border:'none',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}}>
                            Проверить
                          </button>
                        )}
                        <button onClick={ev=>{ev.stopPropagation();setDeleteEntry(e)}}
                          style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:6,background:'none',color:'#EF4444',border:'1px solid #EF444444',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}}>
                          🗑 Удалить
                        </button>
                        <div style={{fontSize:10,color:e.ai_confidence>=85?'#10B981':e.ai_confidence>=60?'#F59E0B':'#EF4444'}}>{e.ai_confidence}% AI</div>
                      </div>
                    </div>
                    ...
                    {expanded===e.id&&(
                      <div style={{padding:'12px 14px 14px',background:'#141929',borderTop:'1px solid #2a3050'}}>
                        ...
                        {e.reviewed_by&&<div style={{fontSize:11,color:'#10B981',marginBottom:6}}>✅ Проверено: {e.reviewed_by}</div>}
                        {e.ai_reasoning&&(
              ...
              <div>
                ...
                {report.needs_review?.length>0&&(
                  <div style={{background:'#1A1200',border:'1px solid #F59E0B44',borderRadius:12,overflow:'hidden',marginBottom:12}}>
                    <div style={{padding:'10px 16px',borderBottom:'1px solid #F59E0B22',fontWeight:700,fontSize:13,color:'#F59E0B'}}>⚠️ Требуют проверки ({report.needs_review.length})</div>
                    {report.needs_review.map(e=>(
                      <div key={e.id} style={{padding:'12px 16px',borderBottom:'1px solid #2a1a00',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <div>
                          <div style={{fontSize:12,fontWeight:700,color:'#e8eaf6',marginBottom:2}}>{e.doc_number?`№${e.doc_number}`:'—'} · {e.counterparty||'—'}</div>
                        ...
                        <div style={{fontSize:13,fontWeight:700,color:'#e8eaf6',flexShrink:0,marginLeft:12}}>{fmt(e.amount,e.currency)}</div>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {report.summary?.total_entries===0&&<div style={{textAlign:'center',padding:40,background:'#181c27',borderRadius:12,border:'1px solid #2a3050',color:'#4a5580'}}>За этот день операций нет</div>}
              </div>
            )}
          </div>
        )}
      ...
      {deleteEntry&&(
        <div style={{position:'fixed',inset:0,background:'#000c',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'#181c27',border:'1px solid #EF444466',borderRadius:16,padding:24,maxWidth:420,width:'100%'}}>
            ...
            <div style={{fontSize:13,color:'#8892b0',marginBottom:6}}>{deleteEntry.doc_number?`№${deleteEntry.doc_number}`:'Без номера'} · {deleteEntry.counterparty||'—'}</div>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={()=>setDeleteEntry(null)}
                style={{flex:1,background:'none',color:'#8892b0',border:'1px solid #2a3050',padding:12,borderRadius:10,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
                Отмена
              </button>
            </div>
          </div>
