...
function ReviewModal({ entry, onClose, onDone }) {
  ...
  async function act(action) {
    ...
    try {
      ...
      if (action === 'correct') { payload.debit_account = debit; payload.credit_account = credit; payload.description = description }
  ...
  const overlayStyle = {
    position:'fixed', inset:0, background:'rgba(30,42,62,0.5)',
    ...
    <div style={overlayStyle}>
      <div style={modalStyle}>
        <div style={{padding:'16px 20px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
          <div>
            <div style={{fontWeight:800, fontSize:15, color:'var(--text)'}}>
              {entry.doc_number ? `№${entry.doc_number}` : 'Документ'} — Проверка
            </div>
        ...
        <div style={{padding:'16px 20px'}}>
          {entry.ai_reasoning && (
            <div style={{background:'var(--ai-light)', borderRadius:'var(--radius)', padding:'10px 14px', marginBottom:16, fontSize:12, color:'var(--ai-text)', border:'1px solid var(--border)'}}>
              🤖 <strong style={{color:'var(--ai)'}}>AI:</strong> {entry.ai_reasoning}
              <div style={{marginTop:4, color: entry.ai_confidence>=80 ? 'var(--success)' : 'var(--warn)', fontWeight:700}}>
          ...
          <div style={{background:'var(--surface2)', borderRadius:'var(--radius)', padding:'12px 16px', marginBottom:16}}>
            ...
            <div style={{display:'flex', alignItems:'center', gap:10, flexWrap:'wrap'}}>
              <div style={{background:'var(--accent-light)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)', padding:'8px 14px'}}>
                <div style={{fontSize:10, color:'var(--accent-text)', fontWeight:700, marginBottom:2}}>ДЕБЕТ</div>
              ...
              <div style={{marginLeft:'auto', textAlign:'right'}}>
                <div style={{fontSize:15, fontWeight:800, color:'var(--text)'}}>{fmt(entry.amount, entry.currency)}</div>
              </div>
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{marginBottom:10}}>
                ...
                <input value={acctSearch} onChange={e=>setAcctSearch(e.target.value)}
                  placeholder="Код или название..." style={{...INP, marginBottom:0}}/>
          ...
          {mode === 'reject' && (
            <div style={{marginBottom:16}}>
              <div style={{fontSize:12, color:'var(--error)', fontWeight:700, marginBottom:10}}>❌ Причина отклонения</div>
              <input value={comment} onChange={e=>setComment(e.target.value)} placeholder="Укажите причину..." style={INP}/>
            </div>
          )}
          ...
          {mode === 'view' && (
            <div style={{display:'flex', gap:10}}>
              <button onClick={()=>act('confirm')} disabled={saving} style={{flex:1, background:'var(--success)', color:'#fff', border:'none', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit', boxShadow:'var(--shadow)'}}>✅ Подтвердить</button>
              <button onClick={()=>setMode('correct')} style={{flex:1, background:'var(--warn-light)', color:'var(--warn)', border:'1px solid var(--warn)', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>✏️ Исправить</button>
              <button onClick={()=>setMode('reject')} style={{flex:1, background:'var(--error-light)', color:'var(--error)', border:'1px solid var(--error)', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>❌ Отклонить</button>
            </div>
          )}
          {mode === 'correct' && (
            <div style={{display:'flex', gap:10}}>
              <button onClick={()=>act('correct')} disabled={saving||!debit||!credit} style={{flex:2, background:(!debit||!credit)?'var(--text4)':'var(--warn)', color:'#fff', border:'none', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:(!debit||!credit)?'not-allowed':'pointer', fontFamily:'inherit'}}>{saving?'Сохраняю...':'✏️ Сохранить'}</button>
              <button onClick={()=>setMode('view')} style={{flex:1, background:'none', color:'var(--text2)', border:'1px solid var(--border)', padding:12, borderRadius:'var(--radius)', cursor:'pointer', fontFamily:'inherit'}}>Назад</button>
            </div>
          )}
          {mode === 'reject' && (
            <div style={{display:'flex', gap:10}}>
              <button onClick={()=>act('reject')} disabled={saving} style={{flex:2, background:'var(--error)', color:'#fff', border:'none', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>{saving?'Отклоняю...':'❌ Подтвердить'}</button>
              <button onClick={()=>setMode('view')} style={{flex:1, background:'none', color:'var(--text2)', border:'1px solid var(--border)', padding:12, borderRadius:'var(--radius)', cursor:'pointer', fontFamily:'inherit'}}>Назад</button>
            </div>
          )}
        </div>
          ...
          {selectMode ? (
            <>
              <button onClick={handleBulkDelete} disabled={deleting||selected.size===0}
                style={{background:selected.size>0?'var(--error)':'var(--text4)', color:'#fff', border:'none', padding:'7px 14px', borderRadius:'var(--radius-sm)', fontSize:12, fontWeight:700, cursor:selected.size>0?'pointer':'not-allowed', fontFamily:'inherit'}}>
                {deleting?'...':`🗑 Удалить (${selected.size})`}
              </button>
              <button onClick={()=>{setSelectMode(false);setSelected(new Set())}}
                style={{background:'none', color:'var(--text2)', border:'1px solid var(--border)', padding:'7px 14px', borderRadius:'var(--radius-sm)', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit'}}>
                Отмена
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={runAutoAll} disabled={postingAll}
                style={{background:postingAll?'var(--text3)':'var(--accent)', color:'#fff', border:'none', padding:'7px 14px', borderRadius:'var(--radius-sm)', fontSize:12, fontWeight:700, cursor:postingAll?'not-allowed':'pointer', fontFamily:'inherit', boxShadow:'var(--shadow-sm)'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, padding:'16px 20px 0'}}>
        {[
          ...
          <div key={s.label} onClick={s.action||undefined}
            style={{background:'var(--surface)', borderRadius:'var(--radius)', padding:'14px 16px', border:`1px solid ${s.action?'var(--warn)':'var(--border)'}`, cursor:s.action?'pointer':'default', boxShadow:'var(--shadow-sm)'}}>
            ...
            {s.action && needsReview > 0 && <div style={{fontSize:10, color:'var(--warn)', marginTop:2}}>↑ нажми для фильтра</div>}
          </div>
        ))}
      </div>
      ...
      <div style={{display:'flex', gap:4, padding:'14px 20px 0'}}>
        {[['journal','📋 Журнал'],['report','📊 Отчёт за день']].map(([key,label]) => (
          <button key={key} onClick={()=>setTab(key)}
            style={{padding:'7px 16px', borderRadius:'var(--radius-sm)', border:tab===key?'none':'1px solid var(--border)', fontFamily:'inherit', fontSize:13, fontWeight:700, cursor:'pointer', background:tab===key?'var(--accent)':'var(--surface)', color:tab===key?'#fff':'var(--text2)', boxShadow:tab===key?'var(--shadow-sm)':'none'}}>
            {label}
          </button>
        ))}
      </div>
          ...
          <>
              ...
              <div style={{display:'flex', gap:8, alignItems:'center', flexWrap:'wrap'}}>
                {[['','Все'],['posted','Проведено'],['needs_review','На проверке'],['rejected','Отклонено']].map(([val,label]) => (
                  <button key={val} onClick={()=>setFilterStatus(val)}
                    style={{padding:'5px 12px', borderRadius:20, border:`1.5px solid ${filterStatus===val?'var(--accent)':'var(--border)'}`, background:filterStatus===val?'var(--accent-light)':'var(--surface)', color:filterStatus===val?'var(--accent-text)':'var(--text2)', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit'}}>
                    {label}
                  </button>
                ))}
                <button onClick={()=>setShowFilters(!showFilters)}
                  style={{padding:'5px 12px', borderRadius:20, border:'1px solid var(--border)', background:'var(--surface)', color:'var(--text3)', fontSize:12, cursor:'pointer', fontFamily:'inherit', marginLeft:'auto'}}>
                  {showFilters?'▲ Свернуть':'▼ Фильтры'}
                </button>
              </div>
              {showFilters && (
                <div style={{display:'flex', gap:10, marginTop:10, flexWrap:'wrap', padding:'12px 14px', background:'var(--surface)', borderRadius:'var(--radius)', border:'1px solid var(--border)', marginTop:8}}>
            ...
            {loading ? (
              ...
              <div style={{textAlign:'center', padding:48, background:'var(--surface)', borderRadius:'var(--radius-lg)', border:'1px solid var(--border)', boxShadow:'var(--shadow-sm)'}}>
                ...
                <button onClick={()=>navigate(`/company/${companyId}/scanner`)}
                  style={{background:'var(--accent)', color:'#fff', border:'none', padding:'10px 20px', borderRadius:'var(--radius)', fontWeight:700, cursor:'pointer', fontFamily:'inherit', fontSize:13, boxShadow:'var(--shadow)'}}>
                  📷 Сканировать
                </button>
              </div>
            ) : (
                ...
                <div style={{display:'grid', gridTemplateColumns:cols, gap:6, padding:'10px 14px', background:'var(--surface2)', fontSize:10, fontWeight:700, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.08em', borderBottom:'1px solid var(--border)'}}>
                  {selectMode && (
                    <div style={{display:'flex', alignItems:'center', justifyContent:'center'}}>
                      <input type="checkbox"
                        onChange={e => e.target.checked ? setSelected(new Set(entries.map(x=>x.id))) : setSelected(new Set())}
                        checked={selected.size === entries.length && entries.length > 0}/>
                    </div>
                  )}
                  <div>№</div><div>Дата</div><div>Документ</div><div>Контрагент</div>
                  <div>Содержание</div><div>Дт</div><div>Кт</div>
                  <div style={{textAlign:'right'}}>Сумма</div><div>Статус</div>
                </div>
                ...
                {entries.map(e => (
                  <div key={e.id}>
                    <div
                      ...
                      style={{display:'grid', gridTemplateColumns:cols, gap:6, padding:'11px 14px', borderBottom:'1px solid var(--border)', alignItems:'center', cursor:'pointer', background:selected.has(e.id)?'var(--accent-light)':e.status==='needs_review'?'var(--warn-light)':expanded===e.id?'var(--surface2)':'var(--surface)', transition:'background 0.1s'}}
                      ...
                      {selectMode && (
                        <div style={{display:'flex', alignItems:'center', justifyContent:'center'}} onClick={ev=>ev.stopPropagation()}>
                      ...
                      <div>
                        {e.doc_number&&<div style={{fontSize:11, fontWeight:700, color:'var(--text)'}}>№{e.doc_number}</div>}
                        {e.doc_type&&<div style={{fontSize:10, color:'var(--text3)'}}>{DOC_TYPE_LABEL[e.doc_type]||e.doc_type}</div>}
                      </div>
                      ...
                      <div>
                        <span style={{fontSize:12, fontWeight:800, color:'var(--accent)', background:'var(--accent-light)', padding:'2px 6px', borderRadius:'var(--radius-sm)'}}>
                          {e.debit_account}
                        </span>
                      </div>
                      <div>
                        <span style={{fontSize:12, fontWeight:800, color:'var(--success)', background:'var(--success-light)', padding:'2px 6px', borderRadius:'var(--radius-sm)'}}>
                          {e.credit_account}
                        </span>
                      </div>
                      ...
                      <div style={{display:'flex', flexDirection:'column', gap:3, alignItems:'flex-start'}}>
                        <span style={{fontSize:10, fontWeight:700, padding:'2px 7px', borderRadius:20, background:S_BG[e.status]||'var(--surface2)', color:S_COLOR[e.status]||'var(--text3)', whiteSpace:'nowrap', border:`1px solid ${S_COLOR[e.status]||'var(--border)'}33`}}>
                          {S_LABEL[e.status]||e.status}
                        </span>
                        {e.status==='needs_review'&&(
                          <button onClick={ev=>{ev.stopPropagation();setReviewEntry(e)}}
                            style={{fontSize:10, fontWeight:700, padding:'2px 7px', borderRadius:'var(--radius-sm)', background:'var(--warn)', color:'#fff', border:'none', cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap'}}>
                            Проверить
                          </button>
                        )}
                        <button onClick={ev=>{ev.stopPropagation();setDeleteEntry(e)}}
                          style={{fontSize:10, fontWeight:700, padding:'2px 7px', borderRadius:'var(--radius-sm)', background:'none', color:'var(--error)', border:`1px solid var(--error)44`, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap'}}>
                          🗑 Удалить
                        </button>
                        <div style={{fontSize:10, color:e.ai_confidence>=85?'var(--success)':e.ai_confidence>=60?'var(--warn)':'var(--error)'}}>
                          {e.ai_confidence}% AI
                        </div>
                      </div>
                    </div>
                  ...
                  <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12}}>
                    {[['Всего',report.summary?.total_entries,'var(--text)'],['Проведено',report.summary?.posted,'var(--success)'],['На проверке',report.summary?.needs_review,'var(--warn)'],['Итого KGS',fmt(report.summary?.total_amount_kgs),'var(--accent)']].map(([l,v,c])=>(
                      <div key={l} style={{textAlign:'center'}}>
                        <div style={{fontSize:20, fontWeight:800, color:c, fontVariantNumeric:'tabular-nums'}}>{v??0}</div>
                        <div style={{fontSize:10, color:'var(--text3)', fontWeight:600, textTransform:'uppercase', marginTop:2}}>{l}</div>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div style={{background:'var(--warn-light)', border:'1px solid var(--warn)', borderRadius:'var(--radius-lg)', overflow:'hidden', marginBottom:12}}>
                    ...
                    {report.needs_review.map(e=>(
                      <div key={e.id} style={{padding:'12px 16px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <div>
                          <div style={{fontSize:12, fontWeight:700, color:'var(--text)', marginBottom:2}}>{e.doc_number?`№${e.doc_number}`:'—'} · {e.counterparty||'—'}</div>
                          <div style={{fontSize:11, color:'var(--text2)'}}>{e.description}</div>
                          <div style={{fontSize:11, color:'var(--text3)', marginTop:2}}>Дт {e.debit?.split(' ')[0]} → Кт {e.credit?.split(' ')[0]} · AI {e.confidence}%</div>
                        </div>
                ...
                {Object.keys(report.totals_by_debit_account||{}).length>0&&(
                  <div style={{background:'var(--surface)', borderRadius:'var(--radius-lg)', border:'1px solid var(--border)', overflow:'hidden', marginBottom:12, boxShadow:'var(--shadow-sm)'}}>
                    ...
                    {Object.entries(report.totals_by_debit_account).map(([acc,amt])=>(
                      <div key={acc} style={{display:'flex', justifyContent:'space-between', padding:'9px 16px', borderBottom:'1px solid var(--border)', fontSize:13}}>
                        <span style={{color:'var(--text2)'}}>{acc}</span>
                        <span style={{fontWeight:700, color:'var(--text)', fontVariantNumeric:'tabular-nums'}}>{fmt(amt)} KGS</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {deleteEntry&&(
        <div style={{position:'fixed', inset:0, background:'rgba(30,42,62,0.5)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:16}}>
          <div style={{background:'var(--surface)', border:'1px solid var(--error)', borderRadius:'var(--radius-lg)', padding:24, maxWidth:420, width:'100%', boxShadow:'var(--shadow-lg)'}}>
            ...
            <div style={{fontSize:13, color:'var(--text2)', marginBottom:6}}>{deleteEntry.doc_number?`№${deleteEntry.doc_number}`:'Без номера'} · {deleteEntry.counterparty||'—'}</div>
            <div style={{fontSize:13, color:'var(--text2)', marginBottom:16}}>
              Дт <strong style={{color:'var(--accent)'}}>{deleteEntry.debit_account}</strong> → Кт <strong style={{color:'var(--success)'}}>{deleteEntry.credit_account}</strong> · {Number(deleteEntry.amount).toLocaleString('ru-RU')} {deleteEntry.currency}
            </div>
            <div style={{background:'var(--error-light)', border:'1px solid var(--error)', borderRadius:'var(--radius-sm)', padding:'10px 14px', marginBottom:16, fontSize:12, color:'var(--error)'}}>
              ⚠️ Документ вернётся в статус «Ожидает разноски»
            </div>
            <div style={{display:'flex', gap:10}}>
              <button onClick={()=>handleDelete(deleteEntry.id)} disabled={deleting}
                style={{flex:1, background:'var(--error)', color:'#fff', border:'none', padding:12, borderRadius:'var(--radius)', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>
