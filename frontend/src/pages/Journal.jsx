    ...
    <div style={{position:'fixed',inset:0,background:'rgba(30,42,62,0.5)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',width:'100%',maxWidth:560,maxHeight:'90vh',overflowY:'auto',boxShadow:'var(--shadow-lg)'}}>
        ...
        <div style={{padding:'16px 20px'}}>
          ...
          {mode==='view'&&<div style={{display:'flex',gap:10}}><button onClick={()=>act('confirm')} disabled={saving} style={{flex:1,background:'var(--success)',color:'#fff',border:'none',padding:12,borderRadius:'var(--radius)',fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow:'var(--shadow)'}}>✅ Подтвердить</button><button onClick={()=>setMode('correct')} style={{flex:1,background:'var(--warn-light)',color:'var(--warn)',border:'1px solid var(--warn)',padding:12,borderRadius:'var(--radius)',fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>✏️ Исправить</button><button onClick={()=>setMode('reject')} style={{flex:1,background:'var(--error-light)',color:'var(--error)',border:'1px solid var(--error)',padding:12,borderRadius:'var(--radius)',fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>❌ Отклонить</button></div>}
