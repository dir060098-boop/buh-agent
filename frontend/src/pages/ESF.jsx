      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <button onClick={() => {
            ...
            if (dateFrom) params.date_from = dateFrom
            if (dateTo)   params.date_to   = dateTo
            esfApi.exportBook(companyId, params)
          }}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
              <button onClick={handleAdd}
                disabled={!form.esf_number || !form.esf_date || !(tab === 'incoming' ? form.supplier_name : form.buyer_name) || !(parseFloat(form.amount) > 0) || saving}
                style={{ flex: 2, background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', padding: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: (!form.esf_number || !(tab === 'incoming' ? form.supplier_name : form.buyer_name) || !(parseFloat(form.amount) > 0)) ? 0.5 : 1 }}>
